...
ar_craighead <- subset(arkansas, subregion %in% c("craighead"))
ar_map <- ggplot() +
geom_polygon(data = arkansas, aes(x=long, y = lat, group = group), fill = NA, color = "black") +
geom_polygon(data = ar_craighead, aes(x = long, y = lat, group = group), fill = "black") +
theme_map() +
coord_fixed(1.3)
ar_map
```
...
```{r}
ar_other_counties <- subset(x= us_arkansas, subregion %in% c("arkansas","randolph"))
```
...
arkansas <- map_data("county", "arkansas")
ar_craighead <- subset(arkansas, subregion %in% c("craighead"))
ar_other_counties <- subset(arkansas, subregion %in% c("arkansas", "randolph"))
...
```{r}
states <- st_read(here::here("Data/shape_file", "cb_2017_us_state_500k.shp"),
...
### Chropleth Map: Your Turn

## Import Data
```{r}
lyme_disease_raw <- read_csv(file = here::here("Data","lyme_disease.csv"),skip = 1,na = c("-"))
lyme_disease_raw
```
...
### Merge the fips data frame to Lyme Disease data
...
### Custom gradient added axis labels, save to base_map
...
### get bathymetric data for the Caribbean Sea
```{r}
...
### plot base bathymetric map, save to base_map_2
```{r}
